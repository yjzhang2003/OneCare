...
- 面向比赛评审的品牌展厅式主页，以一条服务信号串联四个角色视角；
- 可点击和键盘切换的用户、客服、工程师与后台四视角 Demo；
...
当前版本用于呈现万护 OneCare 的产品方向与服务闭环故事。页面中的设备风险、服务队列和能力模块均为静态方案预览，尚未接入真实 IoT、VOC、客服、工单、配件、回访或 AI 服务。

当前主页采用参考海信官网的黑白品牌展厅视觉：顶部和页尾为黑色，主体为白色，使用 MiSans、海信官方大场景图、海信智能冰箱产品图、药丸形文字按钮、圆形图标按钮与白色圆角内容卡。图片与字体来源及处理记录见 `public/images/hisense/SOURCES.md`。所有业务结果仍是方案目标，不是生产指标。

四个角色的独立页面仍是后续阶段，其中用户视角计划包含简单的 AI 聊天智能体演示；当前主页只提供可切换的视角概览，不把未实现页面链接或 AI 对话描述为已经完成。
