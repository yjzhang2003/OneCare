...
四个角色当前位于“四个视角”一级页面内，通过二级全屏横向切换演示同一条服务上下文。用户端在桌面保持手机尺寸，在移动端直接适配设备宽度；对话通过左侧“万护助手”和右侧“我”的身份、头像、气泡、时间与状态呈现主动提醒、用户回复、AI 预诊和预约确认。客服、工程师和后台使用共享工作台外壳与角色专属布局。对话和预约仍是浏览器内的确定性模拟；自由文本 AI、真实身份权限和独立角色路由仍属于后续阶段。
