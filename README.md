...
- 登录后可见的运营工作台（`/`，租户成员）：指标条、情绪极性 / 问题维度 / 渠道三块分布、以及可筛选可操作的工单列表。工单列表展示真实用户原文与真实负责人姓名——门禁正是为了让这件事成立，运营看不到真实反馈就没法干活。读取失败时显示明确的「指标暂不可用」而不渲染任何数字，这与「Base 为空如实显示 0」是两种不同的展示。时间按固定 +08:00 呈现，不读运行环境时区。
  ...
  **五个队列标签页**（待处理 / 超时风险 / 未分配 / 分析异常 / 全部，各带计数）、七类筛选、全文搜索、四种排序、每页 50 条分页、停留时长与超时标记。点击记录编号或整行会进入独立工单详情页；详情页隐藏队列侧栏，以章节导航、主要内容和处理上下文组成三栏，返回时恢复进入前的队列、七类筛选、搜索、排序和页码。列表现场全部由受控 URL 查询参数驱动、服务端渲染，所以粘一个链接就能复现同事看到的那一屏。超时判据用的是**假设 SLA 72 小时**，不是海信给的合同值，界面上标注了这一点。
