...
- **建群与幂等（已接入生产路由）**：`resolveWarRoomAction`（`src/features/feishu-bot/war-room-actions.ts`）读记录、按「操作者是负责人**或**兜底人」校验（比四个状态动作「仅负责人」更宽，因为批准升级本就是兜底人的职权）、按 `协同群 ID` 列的三态（空 → 建群；`oc_*` → 已存在；常量 `declined` → 已拒绝）决定动作；建群成功立刻回写 `chat_id`，随后把工单卡（含**完整用户原文**、不脱敏、不截断——群成员是被点名拉进来处理这条投诉的人，看不到原文没法处理）发进新群。群名规则为 `VOC-{记录编号后 6 位}-{品类}-{严重度}`，成员为该工单负责人全体、点击操作者与机器人自身去重后建群，建群接口为 `POST im/v1/chats`。这部分逻辑都是纯函数与独立单元测试覆盖的模块（`war-room-actions.test.ts`、`naming.test.ts`）。`app/api/feishu/events/route.ts` 的 `createResolveAction` 现在把 `voc_open_war_room`/`voc_decline_war_room` 在送进 `resolveVocCardAction` 之前分流出来，改发给 `resolveWarRoomAction`——两个动作从此不再看到严格的「仅负责人」校验，也不再落进 `resolveVocCardAction` 里那个「该操作暂不支持」的惰性兜底分支（那个分支本身没有改动，仍然留给未来真的忘记接线的新动作用）。`fallbackOpenIds`/`createChat`/`sendToChat` 三个注入分别复用 `app/api/voc/analyze/route.ts` 已经导出的 `listOwnerRules`/`fallbackOwnerOpenIds`（同一张负责人表，同一份兜底名单，不重新实现一遍）、`src/features/feishu-bot/chat-client.ts` 的 `createWarRoomChat`、`src/features/feishu-bot/client.ts` 的 `sendFeishuMessage`（走 `chatId` 分支）。回调仍在同一次同步响应里完成——`resolveWarRoomAction` 内部的建群失败/写列失败各自有专属 toast 文案，路由层没有在外面再包一层 try/catch 把它们吞成一句笼统的错误。
