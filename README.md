...
- 登录后可见的运营工作台（`/`，租户成员）：指标条、情绪极性 / 问题维度 / 渠道三块分布、以及可筛选可操作的工单列表。工单列表展示真实用户原文与真实负责人姓名——门禁正是为了让这件事成立，运营看不到真实反馈就没法干活。读取失败时显示明确的「指标暂不可用」而不渲染任何数字，这与「Base 为空如实显示 0」是两种不同的展示。时间按固定 +08:00 呈现，不读运行环境时区。
  ...
  **五个队列标签页**（待处理 / 超时风险 / 未分配 / 分析异常 / 全部，各带计数）、七类筛选、全文搜索、四种排序、每页 50 条分页、停留时长与超时标记、点行展开工单详情。全部由 URL 查询参数驱动、服务端渲染，所以粘一个链接就能复现同事看到的那一屏。超时判据用的是**假设 SLA 72 小时**，不是海信给的合同值，界面上标注了这一点。
  ...
  **源数据里没有任何用户身份**（无手机号、无账号、无用户名），所以「统一服务事件」三个关联键中，实际存在的是 `记录编号` 与 `来源单号`；用户 ID 与设备 ID 在这份数据上无法诚实构造，不做伪造。原先映射但从未读取、且 3628 条全空的 `用户标识` 字段映射已移除。
  ...
  唯一的 `"use client"` 是详情面板的动作岛（Arco Design）。它无法判断当前访客是不是负责人——那是 per-viewer 的，而数据来自所有访客共享的缓存条目，要么泄露 open_id 要么按人污染缓存——所以按钮对所有人显示、服务端是唯一权威，面板上明说非负责人点击会被拒绝；
