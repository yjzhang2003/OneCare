...
- 登录后可见的运营工作台（`/`，租户成员）：指标条、情绪极性 / 问题维度 / 渠道三块分布、以及可筛选可操作的工单列表。工单列表展示真实用户原文与真实负责人姓名——门禁正是为了让这件事成立，运营看不到真实反馈就没法干活。读取失败时显示明确的「指标暂不可用」而不渲染任何数字，这与「Base 为空如实显示 0」是两种不同的展示。时间按固定 +08:00 呈现，不读运行环境时区。
  ...
  **五个队列标签页**（待处理 / 超时风险 / 未分配 / 分析异常 / 全部，各带计数）、七类筛选、全文搜索、四种排序、每页 50 条分页、停留时长与超时标记。点击记录编号或整行会进入独立工单详情页；详情页隐藏队列侧栏，以章节导航、主要内容和处理上下文组成三栏，返回时恢复进入前的队列、七类筛选、搜索、排序和页码。列表现场全部由受控 URL 查询参数驱动；参数解析、筛选、排序和分页在服务端完成，客户端只接收当前页数据，所以粘一个链接就能复现同事看到的那一屏。超时判据用的是**假设 SLA 72 小时**，不是海信给的合同值，界面上标注了这一点；当停留时长无法计算时，时长和超时状态都显示 `—`，不会误报「未超时」。

  **网页可以改状态、可以认领**（`POST /api/voc/tickets/[recordId]/action`）。界面不给自由选目标状态的下拉，只给当前状态下合法的动作——`availableActions` 探测已有的状态机而不是重抄规则表，所以非法跳转无法表达，会被守卫拒绝的动作（重试到顶、`需建单` 缺负责人）直接不显示。`打标成功` / `打标失败` 属于打标流水线，网页与 HTTP 边界都拒绝。鉴权两个动作不同：**状态流转要求是负责人本人**（与卡片路径 `resolveVocCardAction` 逐字一致，网页不比卡片宽松）；**认领只能填空、不能顶替**（已有负责人一律拒绝，改派走多维表格的人员选择器）。因此「先认领、再建单」是 `已分析` 这个死胡同唯一的出路——流水线在没解析出负责人时会卡在那里，而卡片侧没有任何动作映射到 `需建单`。
  ...
  **改派负责人（2026-08-14）**：通讯录接口此前全部返回 `99991672` 未开通，所以代码无法把姓名解析成 open_id——这正是网页只能「认领给自己」、而指派他人必须去多维表格人员选择器的原因。权限开通后实测 `contact/v3/scopes` 能列出应用可见成员的 open_id、`contact/v3/users/batch` 能取到姓名，`src/features/directory/members.ts` 成为唯一解析入口，工作台因此有了「改派」。被指派人的 open_id 由服务端对着目录校验后才写入，姓名也由服务端解析，不采信浏览器传来的名字。多维表格的人员选择器仍然可用，只是不再是唯一入口。
  ...
  工作台列表与完整工单详情工作区都是 Arco 客户端展示面，详情工作区复用聚焦的 `WorkbenchActions` 动作组件；筛选、排序和分页仍由服务端派生。客户端无法判断当前访客是不是负责人——那是 per-viewer 的，而数据来自所有访客共享的缓存条目，要么泄露 open_id 要么按人污染缓存——所以按钮对所有人显示、服务端是唯一权威，页面上明说非负责人点击会被拒绝；
