...
- 可点击和键盘切换的四套沉浸式角色 Demo：手机尺寸的用户 AI 服务助手、客服智能坐席、工程师一次上门工作台与后台 VOC 闭环驾驶舱；
...
当前版本用于呈现万护 OneCare 的产品方向与服务闭环故事。四个角色共享静态案例 `OC-240718-037`，用户可以完成预诊与服务安排，客服可以生成工单，工程师可以确认携件并完成服务，后台可以切换 VOC 主题并创建改善任务。所有回复、设备信号、工单、配件、指标和状态变化均为浏览器内的确定性模拟，尚未接入真实 IoT、VOC、客服、工单、配件、回访或 AI 服务。

当前网站采用参考海信官网的黑白品牌展厅视觉：顶部和页尾为黑色，主体为白色，使用 MiSans、海信官方大场景图、海信智能冰箱产品图、药丸形文字按钮、圆形图标按钮与白色圆角内容卡。一级页面标题统一使用中文，Top Bar 切换会保留 URL Hash、浏览器前进后退和深链恢复。图片与字体来源及处理记录见 `public/images/hisense/SOURCES.md`。所有业务结果仍是方案目标，不是生产指标。

四个角色当前位于“四个视角”一级页面内，通过二级全屏横向切换演示同一条服务上下文。用户端在桌面保持手机尺寸，在移动端直接适配设备宽度；客服、工程师和后台使用共享工作台外壳与角色专属布局。自由文本 AI、真实身份权限和独立角色路由仍属于后续阶段。
