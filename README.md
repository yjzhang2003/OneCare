...
- 登录后可见的运营工作台（`/`，租户成员）：指标条、情绪极性 / 问题维度 / 渠道三块分布、以及可筛选可操作的工单列表。工单列表展示真实用户原文与真实负责人姓名——门禁正是为了让这件事成立，运营看不到真实反馈就没法干活。读取失败时显示明确的「指标暂不可用」而不渲染任何数字，这与「Base 为空如实显示 0」是两种不同的展示。时间按固定 +08:00 呈现，不读运行环境时区。
  ...
  **展厅首页的数字也走 Postgres 了（2026-08-14）**：它是最后一个还在读多维表格全表的界面，而且恰好是外部访客第一眼看到的那一屏。同机同数据实测：整页 2.5 秒（缓存冷时更差）降到 **125ms**，其中聚合本身 117–124ms。用的是控制台「数据概览」那同一个聚合，所以两处数字现在是同源一致，而不是各跑一遍同样的内存归约恰好相等。`app/page.test.tsx` 里那两个 Bitable 读取器都被换成会抛错的 mock：一个守「匿名访客不许碰逐单详情」，一个守这条性能回退。
