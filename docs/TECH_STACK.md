...
- `src/features/feishu-bot/war-room-actions.ts`: `resolveWarRoomAction`, the pure approval/creation logic — record lookup, "owner or fallback" authorization (looser than the four state actions' owner-only rule, because approving an escalation is the fallback owner's job), idempotence over the three-state column, group creation, and the opening ticket card send. Fully unit-tested in `war-room-actions.test.ts`; **not called from any production route** — see "Known gap" below.
...
- `app/api/feishu/events/route.ts`: `createAnswerGroupQuestion` (wired into the `group_question` outcome that `event-handler.ts`'s `parseFeishuEvent` now emits for group text that actually mentions the bot) and the closure-archival branch of `resolveVocCardAction`'s dispatch (`readTranscript`/`summarise` injected as `readWarRoomTranscript`/`summariseClosure`) are both live in `defaultDependencies` — these two paths do run in production once a ticket already has a real war room chat id.

**Known gap, load-bearing**: the two approval actions above are not reachable from a real webhook today. `createResolveAction` in `app/api/feishu/events/route.ts` routes every `VocCardAction` — the four state transitions and the two war-room actions alike — through `resolveVocCardAction` (`src/features/feishu-bot/card-actions.ts`), which enforces strict owner-only authorization (a fallback owner who is not also a ticket owner is rejected before this even matters) and has no transition defined for `voc_open_war_room`/`voc_decline_war_room` (`ACTION_TO_TRANSITION` is a `Partial`; an action missing from it returns the inert toast `该操作暂不支持`). A real click today creates nothing and writes nothing. Closing this gap means adding a branch — in `app/api/feishu/events/route.ts` or `card-actions.ts` — that recognizes these two actions and dispatches to `resolveWarRoomAction` instead of `resolveVocCardAction`. Until then, spec §12 acceptance items 2–5 cannot pass, and the in-group Q&A / closure-archival paths above can only be exercised against a `协同群 ID` written by hand.
...
- wiring the war room's two approval card actions (`voc_open_war_room` / `voc_decline_war_room`) into a production route (see "VOC War Room" above — the logic exists and is tested, the dispatch does not);
