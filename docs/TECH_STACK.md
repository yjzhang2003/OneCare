...
The 万护 OneCare TypeScript web baseline and Feishu custom-app login were implemented on 2026-07-17. A guided Feishu experience page and stateless employee bot webhook were added on 2026-07-18. On 2026-07-19 the bot was upgraded locally to an all-Card-2.0 workbench with verified button callbacks; production deployment, callback subscription, app-version publication and real-member acceptance are still required. A VOC closed loop was added on 2026-08-11: real VOC feedback lands in a Feishu Bitable; dual-track AI tagging (a Feishu aily skill, or a Bitable AI field shortcut, selected by `TAGGING_PROVIDER`) fills one shared tagging contract; a Cron-driven, resumable shard job at `/api/voc/analyze` retags failed records under a retry ceiling; a Feishu card workflow carries a ticket through `待跟进 → 跟进中 → 待闭环 → 已闭环` behind a synchronous triple check. On the same day the landing page was split by identity: signing in from the Feishu client (via `/enter`, the app's web homepage) lands on an operations workbench at `/` with real per-ticket detail, queue tabs, filters, search, sorting and pagination, and — since 2026-08-13 — state transitions and ticket claiming from the web itself via `POST /api/voc/tickets/[recordId]/action` (route handler, never a Server Action; see README), an external visitor still gets the pitch showcase, `/api/voc/dashboard` requires a session, and the formerly public `/dashboard/voc` page became a 307 toward `/`. On 2026-08-12 a VOC war room was added on top of the closed loop: a high-severity record escalates to a Feishu group-chat workflow with an approval card, an in-group free-text Q&A skill grounded in Bitable facts, and a closure-time transcript summary (see "VOC War Room" below and `docs/superpowers/specs/2026-08-12-onecare-voc-warroom-design.md`); the escalation card's own two approval buttons are implemented, unit-tested, and wired to the production card-callback route, so a real click creates a group. On 2026-08-13 a Neon Postgres mirror (`sin1`) became the primary store: every read surface answers from it, writes land there first and are pushed to the Bitable after (a failed push keeps `pending_push` and is retried by the daily Cron), and the workbench's own numbers are aggregate SQL rather than a pass over 3628 rows — a full-table read of 8.9s became a 947ms page query and 116ms counts. On 2026-08-14 the ticket detail page moved inside the console shell (same sider as the profile pages) and grew a manual `POST /api/voc/tickets/[recordId]/analyze`, which runs the Cron shard's own tagging logic over one record on an operator's click. The same day the demo dataset's time axis and processing state were re-seeded (`src/features/demo/timeline.ts`): the import carried dates without clock times and sat six months in the past, so every 反馈时间 was 00:00 and 3612 of 3628 rows read 已超时. The ten days keep their relative shape and slide to the present, most rows get tags decided by the real `triage()`, the 19 rows the live aily skill tagged are preserved whole, and every synthesized row is stamped 打标来源 = `demo-seed`. AI 摘要 and AI 回复话术 are never synthesized. See README for what is real and what is not. The current application remains a single-enterprise demonstration; real IoT device data, other service-system integrations (customer service, work orders, parts, follow-up), and production activation of the employee bot workbench remain unimplemented.
...
Feishu callbacks and Bitable reads have a stricter network placement requirement than the rest of the site. `vercel.json` deploys `app/api/feishu/events/route.ts` and every page or API entry point that directly or through a shared reader reaches Bitable to Hong Kong (`hkg1`), including `/` and `/workbench/tickets/[recordNumber]`; unrelated Node.js routes retain the project default. `vercel-config.test.ts` derives this set from source markers and normalizes platform-specific relative paths to `/` before looking up Vercel function keys, so the invariant works on Windows as well as POSIX. Deployment inspection must confirm the emitted functions' regions before production callback and workbench validation.
