...
- `PerspectiveTabs` 持有唯一共享 reducer，四个工作台不再各自维护互相脱节的业务阶段；
- 用户完成 AI 预诊后先看到三步知识库排查建议，可选择“问题已解决”或“仍需人工服务”；
- 转人工、客服建单、工程师携件确认、工程师完成服务与后台改善任务按前置条件依次解锁；
- 用户、客服、工程师和后台的“重新演示”均重置同一案例，当前角色 Tab 保持不变；
- 四视角标题和工作台主文案已改为业务化表达，界面中不再使用“上下文”作为产品主叙事；
- 用户进度只保留圆点与状态文字，步骤文字下方的连接横线已移除；
- 所有能力仍是浏览器内的确定性模拟，不代表真实系统已接入。

完整自动化验证通过：`npm test` 通过 17 个测试文件、66 个测试；`npm run test:runtime` 完成生产构建并通过 3 个运行时测试；`npm run lint`、`npm run typecheck`、独立的 `npm run build` 与 `git diff --check` 均退出码为 0；`npm audit --omit=dev` 返回 `0 vulnerabilities`。首次全量 Lint 发现后台在 effect 内同步重置本地主题，已改为由共享阶段控制组件重建边界，定向测试和全量验证随后均通过。

本地生产构建已在 `1440 × 900` 与 `390 × 844` 验收完整人工服务路径：

- 桌面端从 `detected`、`selfHelp`、`serviceRequested` 到 `improvementCreated`，场景固定为 `y=193.765625px`、`height=706.234375px`，手机固定为 `y=213.765625px`、`height=666.234375px`，操作槽固定为 `y=759px`、`height=44px`，三者阶段极差均为 `0px`；
- 移动端场景与手机在所有抽样阶段固定为 `y=244.140625px`、`height=575.859375px`，底部为 `820px < 844px`；操作槽 `y=702 / 701 / 701 / 701px`、`height=44px`，阶段极差为 `1px`；
