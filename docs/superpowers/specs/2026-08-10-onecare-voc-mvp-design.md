...
- ~~**网站 OAuth 登录与角色**：评委已在同一企业内，登录不再是访问前提~~ —— **本条已于 2026-08-11 被 `2026-08-11-onecare-workbench-design.md` §11 反转。** 当时这么写的前提是：站上没有任何承载记录级数据的页面，公开的聚合看板只有数字，登录因此不带来安全收益，只带来一道门槛。现在有了——工作台展示真实用户原文与真实负责人姓名，登录成为访问前提，`/api/voc/dashboard` 也随之门禁。外部访客看方案展示厅，交付叙事仍不依赖登录。卡片鉴权这半句不变且仍然成立：卡片身份来自事件签名保证的 `event.operator.open_id`，从不读网站会话
...
> **字段形状已校准**（2026-08-10，对真实 Base 写入并读回）。四条与直觉相反、且全部静默失败的结论：
>
> - **数字字段读回字符串**（`"2"`），不是数字。`重试次数` 若按数字判读会恒为 0，重试上限永不触发。
...
>
> **仍待校准**：源数据的列名与取值范围以常见 VOC 周报结构推定，真实 xlsx 到手后回填。若不一致，改动限于 `field-map.ts`、脱敏规则及其测试，不影响状态机、契约、指标。
...
**流转字段**：流程状态、负责人、建单时间、跟进记录（追加式）、闭环时间、闭环结论

**实现落地**（2026-08-11 回填）：状态序号不落 Base 列。它由 `VOC_STATE_SEQUENCE`（`src/features/voc/service-event.ts`）从 `流程状态` 的字符串值实时推导，只在内存里用于 §4.4 的并发判断，不需要、也没有单独的 Base 字段——多一列同步字段只会多一个和 `流程状态` 打架的风险。
...
### 3.6 导入约定（运营侧操作，非程序化去重）

**实现落地**（2026-08-11 回填）：本节原设计的程序化去重（业务唯一键 `渠道 + 反馈时间 + 原始内容哈希`）没有对应代码。原因是 §1.4 已经把导入方式定为运营在多维表格里用自带「导入 Excel」手动完成，而不是走仓库里的导入路由——没有导入路由，去重逻辑就没有地方挂载。

**操作约定**：导入前必须确认 `VOC 记录表` 为空，或先清空表内已有记录，再执行「导入 Excel」，以此替代程序化去重。这把「不重复导入」的责任交给操作步骤而不是代码，因此验收（§14 第 1 条）也相应改为「Base 中条数与源 xlsx 行数逐一核对一致」，而不是「去重后条数一致」。

若后续需要支持增量导入、多批次合并或撤销重放，需要另开规格并补一条程序化去重路径；本期不做。
...
**改为可恢复分片**：`/api/voc/analyze` 每次只取 N 条 `流程状态 = 待分析` 的记录处理完即返回，由 Vercel Cron 推进一片。天然满足 100 次/分钟限流，天然幂等（按状态过滤，成功的不会被重取），天然可恢复（中断后下次继续）。

批大小由「单批端到端 ≤ 20s」实测反推，初值 5 条，实测后调整。官方对该接口未给出超时时间，**我们自己设 25s 上限**。

**实现落地**（2026-08-11 回填）：本节原写「由 Vercel Cron 每分钟推进一片」，与实现不符。Vercel Hobby 计划的 Cron 频率上限是每天一次，`vercel.json` 因此配的是 `{ "path": "/api/voc/analyze", "schedule": "0 18 * * *" }`（每天 18:00 一次），而不是每分钟。分片设计本身（幂等、可恢复、单片超时保护）不受影响——它天然兼容任意调用频率，一天一次同样安全；只是「几分钟内处理完全部待分析记录」的假设不成立，改为「每天一次，累计推进」。如需更高频率，需要升级 Vercel 计划或改用外部触发器，这不是本期范围。
...
**实现落地**（2026-08-11 回填，Task 15）：以下每条标准后附「现状」——本设计文档到 Task 14 为止只交付了代码与测试，Base 本身仍是两表 0 条（字段/枚举已建好，见 §3.1），因此凡依赖真实数据才能观测的条目，现状标注为「代码已实现，待真实数据后验收」，不视为已通过。

1. 真实 VOC 数据在 Base 中可见，条数与源 xlsx 行数一致（导入前确认 Base 为空，见 §3.6 的操作约定，程序化去重未实现）。
   **现状：未通过——xlsx 尚未导入，两表均为 0 条。待运营完成「导入 Excel」后据此核对条数。**
2. 打标跑通，成功率与失败原因可查；分析失败的记录可被下一片 Cron 重取。
   **现状：代码与单测已覆盖**（`src/features/tagging/contracts.ts` 契约校验、两条打标提供方、`app/api/voc/analyze/route.ts` 的 `buildPendingShard` 重试合并逻辑），**但因 Base 无数据，尚未有真实一轮打标可供观测；首次 Cron 触发（`vercel.json`：`0 18 * * *`，Vercel Hobby 计划限每日一次，并非每分钟）在导入完成后才能验证。**
3. 至少一条中差评走完 `待跟进 → 跟进中 → 待闭环 → 已闭环`，全程在飞书卡片内操作，状态写回 Base。
   **现状：状态机、三重校验与工单卡渲染均已实现并有单测**（`src/features/voc/service-event.ts`、`src/features/feishu-bot/card-actions.ts` 的 `resolveVocCardAction`、`cards.ts` 的 `createVocTicketCard`），**真实一轮人工点击验收待 Base 有数据后进行。**
4. **非负责人点击按钮，在同步响应内被拒绝并收到明确 toast**，状态不变。
   **现状：已实现并有单测**——`resolveVocCardAction` 在读到记录后立即比对 `operatorOpenId` 与 `ownerOpenIds`，不匹配直接返回 `errorToast`，`updateRecord` 从未被调用。真实飞书环境下的人工复核仍需等 Base 有数据。
5. 公开看板显示真实聚合，每个数字可在 Base 中点开对账；页面响应不含 `原始内容`。
   **现状：已实现**——`/dashboard/voc` 与 `/api/voc/dashboard` 都经由 `getVocDashboardMetrics` 读取真实 Base，聚合类型（`VocMetricsInput`/`DashboardRecord`）本身不含 `原始内容`/`record_id` 字段，不是运行时过滤。Base 当前 0 条记录时页面如实显示 `0`，这与读取失败时的「指标暂不可用」是两种不同的展示（见 §6.4）。
6. `/api/voc/analyze` 无 `CRON_SECRET` 返回 401。
   **现状：已实现并有单测**（`createAnalyzeRoute` 在触碰任何依赖前先比对 `Authorization` 头）。
7. `npm test`、`npm run test:runtime`、`npm run lint`、`npm run typecheck`、`npm run build`、`npm audit --omit=dev` 全绿。
   **现状：前五条绿；`npm audit --omit=dev` 非零退出，是分支起点 `b1daba4` 就存在的既有状况（`next@16.2.10` 自身及其依赖树 `postcss`/`nanoid`/`sharp` 的 4 项传递漏洞），本期裁决不处置，理由与影响面见 `README.md`「验证」一节。**
8. 提交文档中已实现与规划中的边界表述准确；人效基线标注为假设；并发与权限信任边界如实说明。
   **现状：本条即 Task 15 本身——`README.md`、`AGENTS.md`、`docs/TECH_STACK.md` 已按本文件回填，四条边界（并发 best-effort、权限信任边界、人效为折算值且不给年化金额、Base 为空与读取失败的展示区分）均已写入 `README.md`「VOC 闭环配置」一节。**
9. 不剪辑视频完成录制。
   **现状：不在本仓库范围内，由用户在提交前完成录制。**
10. `README.md`、`AGENTS.md`、`docs/TECH_STACK.md` 三处措辞与实际一致。
    **现状：本次（Task 15）已同步三处文档；后续若实现面再变化，需要再次回填，而不是留给下一次评审自己发现矛盾。**
